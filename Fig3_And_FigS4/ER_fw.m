...
N = 100; p = 0.1; mu = 1; MaxPer = 1;
alpha = 0.5; c = 0.2; K = 1; delta = 1/5; theta = 0.95;
PerStep = 0.02; Step = MaxPer / PerStep + 1; loop = 5;
x0h = ones(N,1) * 2; Data = zeros(loop, Step, 5);
eta_eff_c = ((K + c -2 * sqrt(K * c)) * alpha) / (K * delta) * ones(Step,1);

parfor i = 1 : loop
    A = 1 * Net_ER(N,p); A0 = normrnd(mu, (mu / 5), [N, N]) .* A; Datatemp = zeros(Step, 5);
    for j = 1 : Step
...
[D1,D2,D3] = size(Data); t = toc; disp(t); save ./Data/Data_ER_fw